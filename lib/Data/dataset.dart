...
String appId = "f0b5dabff7d57c02f3a43ae9b03ef164";
...
Future<List> fetchData(String lat,String lon,String city) async{
  ...
  if(response.statusCode==200){
    ...
    for(var i=0;i<4;i++){
      ...
      cuacaHariini.add(hourly);
    }

    //Cuaca Besok
    var harian = res["daily"][0];
    Weather cuacaBesok = Weather(
        max: harian["temp"]["max"]?.round()??0,
        min:harian["temp"]["min"]?.round()??0,
        image: findIcon(harian["weather"][0]["main"].toString(), true),
        name:harian["weather"][0]["main"].toString(),
        wind: harian["wind_speed"]?.round()??0,
        humidity: harian["rain"]?.round()??0,
        chanceRain: harian["uvi"]?.round()??0
    );
    ...
    return [currentTemp,cuacaHariini,cuacaBesok,sevenDay];
  }
